...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        for (Class<?> clazz = pageClass; clazz != null; clazz = clazz.getSuperclass()) {
            for (Method met : clazz.getDeclaredMethods()) {
                if (met.getName().equals(route.getAction())) {
                    method = met;
        ...
        if (lang != null) {
            if (lang.length() != 2  || !lang.matches("[a-z]+")) {
                // throw new ServletException("Can't foud langue [pageClass="
                //        + pageClass + "]");
            } else {
        ...
        Template template;
        try {
            template = newTemplate(pageClass.getSimpleName() + suf + ".ftlh");
    ...
    private Template newTemplate(String templateName) throws ServletException {
        ...
        if (sourceConfiguration != null) {
            try {
                template = sourceConfiguration.getTemplate(templateName, new Locale("en"));
